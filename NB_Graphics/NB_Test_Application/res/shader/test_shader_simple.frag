...
{
	//Ambient
	vec3 ambient_light = light.ambient_strength * light.color * vec3(texture(texture_map, vertex_uv));

	return ambient_light + max(dot(vertex_dir, -light.direction), 0.0f) * light.strength * vec3(texture(texture_map, vertex_uv)) * light.color;
}
